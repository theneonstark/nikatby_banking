...
function Ladger() {
    const [activeStatement, setActiveStatement] = useState("collection");
    ...
    const filterInvoices = () => {
        const filteredData = invoices.filter((invoice) => {
            ...
            const matchesReport =
                !selectedReport || invoice.Operator === selectedReport;

            return (
                matchesDate && matchesSearch && matchesStatus && matchesReport
            );
        });
    ...
    return (
        <div>
            <Card className="flex gap-4 py-4 px-4">
                <Button
                    className={`${
                        activeStatement === "collection" ? "bg-red-400" : ""
                    }`}
                    onClick={() => setActiveStatement("collection")}
                >
                    COLLECTION-WALLET
                </Button>
                <Button
                    className={`${
                        activeStatement === "payout" ? "bg-red-400" : ""
                    }`}
                    onClick={() => setActiveStatement("payout")}
                >
                    PAYOUT WALLET
                </Button>
                <Button
                    className={`${
                        activeStatement === "qrwallet" ? "bg-red-400" : ""
                    }`}
                    onClick={() => setActiveStatement("qrwallet")}
                >
                    QR WALLET
                </Button>
                <Button
                    className={`${
                        activeStatement === "rrwallet" ? "bg-red-400" : ""
                    }`}
                    onClick={() => setActiveStatement("rrwallet")}
                >
                    RR WALLET
                </Button>
            </Card>
                ...
                <div className="flex gap-5">
                    ...
                    {/* <Select value={selectedReport} onValueChange={setSelectedReport}>
                        ...
                        <SelectContent>
                            ...
                            <SelectGroup>
                                <SelectItem value="SelectProduct">
                                    Select Product
                                </SelectItem>
                            </SelectGroup>
                        </SelectContent>
            ...
            <div>
                {activeStatement === "collection" && (
                    <CollectionWalletStatement invoices={filteredInvoices} totalAmount={totalAmount}/>
                )}
                {activeStatement === "payout" && <MainWalletStatement invoices={filteredInvoices} totalAmount={totalAmount}/>}
                {activeStatement === "qrwallet" && <QrWalletStatement invoices={filteredInvoices} totalAmount={totalAmount}/>}
                {activeStatement === "rrwallet" && <RrWalletStatement invoices={filteredInvoices} totalAmount={totalAmount}/>}
            </div>
        </div>
    );
}
